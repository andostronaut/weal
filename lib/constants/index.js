...
const CMD_PATH_UNIX_NVM = `$HOME/.nvm/versions/node/${process.version}/lib/node_modules/weal`
...
const SHELL = Object.freeze({
  IS_BASH: process.env.SHELL && process.env.SHELL.includes('bash'),
  IS_CMD: process.env.SHELL && process.env.SHELL.includes('cmd.exe'),
  IS_POWERSHELL: process.env.SHELL && process.env.SHELL.includes('powershell.exe')
})

const COMSPEC = Object.freeze({
  IS_CMD: process.env.COMSPEC && process.env.COMSPEC.includes('cmd.exe'),
  IS_POWERSHELL: process.env.COMSPEC && process.env.COMSPEC.includes('powershell.exe')
})
...
const INTERFACE_CMD = Object.freeze({
  ...
  UNIX_NVM: `${NODE} ${HTTP_SERVER_CLI.UNIX_NVM} ${CMD_PATH_UNIX_NVM}/dist -p 3222 -o --silent`,
