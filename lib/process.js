...
app.get('/process/:command', (req, res) => {
  ...
  if (cmd === CURRENT_DIR) {
    if (OS.IS_DARWIN || OS.IS_LINUX) {
      if (SHELL.IS_BASH) {
        exec(CURRENT_DIR_CMD.BASH, (_err, stdout) => res.send(stdout))
      }
    } else if (OS.IS_WINDOWS) {
      if (SHELL.IS_CMD) {
        exec(CURRENT_DIR_CMD.CMD, (_err, stdout) => res.send(stdout))
      } else if (SHELL.IS_POWERSHELL) {
        exec(CURRENT_DIR_CMD.POWERSHELL, (_err, stdout) => res.send(stdout))
